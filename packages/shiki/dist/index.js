...
async function getOniguruma(wasmPath) {
  if (!_onigurumaPromise) {
    ...
    if (isBrowser) {
      ...
      const wasmPath2 = path.join(require$1.resolve("vscode-oniguruma"), "../onig.wasm");
...
function _resolvePath(filepath) {
  if (isBrowser) {
    ...
    if (path.isAbsolute(filepath)) {
      ...
      const { fileURLToPath } = require$1("url");
