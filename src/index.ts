...
export type UserConfigSettings = HighlighterOptions &
  TwoSlashOptions &
...
export const renderCodeToHTML = (
  ...
  if (!highlighter && !storedHighlighter) {
    throw new Error(
      "The highlighter object hasn't been initialised via `setupHighLighter` yet in shiki-twoslash"
    );
  }
...
export const runTwoSlash = (
  ...
  if (
    ...
    !settings.disableImplicitReactImport &&
    !hasReactImport.test(code)
  ) {
