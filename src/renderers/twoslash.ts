...
export function twoslashRenderer(
  lines: Lines,
  options: HtmlRendererOptions & TwoslashShikiOptions,
  twoslash: TwoSlash,
  meta: Meta
) {
  let html = "";
  ...
  const tagsGroupedByLine =
    groupBy(twoslash.tags, (q) => q.line - 1) || new Map();
  ...
  lines.forEach((l, i) => {
    ...
    if (l.length === 0 && i === 0) {
      ...
      l.forEach((token) => {
        let targetedQueryWord:
          | (typeof twoslash.staticQuickInfos)[number]
          | undefined;
        ...
        targetedQueryWord =
          targetedQueryWord ||
          lspResponsesInToken.find(
            (response) => response.text === (queries.length && queries[0].text)
          )!;
        ...
        if (allTokensByStart.length) {
          const ranges = allTokensByStart.map((token) => {
            ...
            if ("targetString" in token) {
              range.classes = "lsp";
              const lspText =
                options.includeJSDocInHover && token.docs
                  ? `${token.docs}\n\n${token.text}`
                  : token.text;
              range["lsp"] = lspText;
            }
          ...
          tokenContent += createHighlightedString(
            ranges,
            token.content,
            targetedQueryWord?.text
          );
        } else {
      ...
      queries.forEach((query) => {
        ...
        switch (query.kind) {
          case "query": {
            ...
            const lspValues = staticQuickInfosGroupedByLine.get(i) || [];
            const targetedWord = lspValues.find(
              (response) =>
                response.text === (queries.length && queries[0].text)
            )!;
            const halfWayAcrossTheTargetedWord =
              (targetedWord &&
                targetedWord.character + targetedWord?.length / 2) - 1 || 0;
            html +=
            ...
            if (!query.completions) {
              ...
              const lis = prefixed
                ...
                .map((c) => {
                  ...
                  const isDeprecated = c.kindModifiers
                    ?.split(",")
                    .includes("deprecated");
    ...
    if (tags.length) {
      tags.forEach((tag) => {
        ...
        switch (tag.name) {
          case "error":
            html += `${errorSVG}<span class='message'>${
              tag.annotation || "N/A"
            }</span>`;
            break;
          case "warn":
            html += `${warningSVG}<span class='message'>${
              tag.annotation || "N/A"
            }</span>`;
            break;
          case "log":
            html += `${logSVG}<span class='message'>${
              tag.annotation || "N/A"
            }</span>`;
            break;
        }
